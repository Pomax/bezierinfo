...
{
	points = new Point[4];
	points[0] = new Point(20,210);
	points[1] = new Point(85,275);
	points[2] = new Point(270,130);
	points[3] = new Point(240,265);
}
...
void drawOffsets(Segment segment) {
	...
	for(int f=0; f<n_offset.length; f++) {
		...
		double x2 = (1-et) * n_offset[f].points[3].getX() + et * f_offset[f].points[3].getX();
		double y2 = (1-et) * n_offset[f].points[3].getY() + et * f_offset[f].points[3].getY();
		
		// interpolation, t-value based
		Bezier3 interpolated = interpolateCubicForT(segments[f], n_offset[f], f_offset[f], n_offset[f].getStartT(), n_offset[f].getEndT());
