...
void drawOffsets(Segment segment) {
	...
	for(int f=0; f<n_offset.length; f++) {
		...
		stroke(0,50);
		line(	(int)n_offset[f].points[0].getX(), (int)n_offset[f].points[0].getY(), 
				(int)f_offset[f].points[0].getX(), (int)f_offset[f].points[0].getY());
		line(	(int)n_offset[f].points[1].getX(), (int)n_offset[f].points[1].getY(), 
				(int)f_offset[f].points[1].getX(), (int)f_offset[f].points[1].getY());
	}
}
