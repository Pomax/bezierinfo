...
(function referrer(l) {
  ...
  // right, continue
  var url = "//pomax.nihongoresources.com/pages/bezierinfo/logger.php";
  ...
  var httpTohttps = function() {
    var loc = window.location.toString();
    if (loc.indexOf('localhost') === -1 && loc.indexOf('http://') !== -1) {
      window.location = loc.replace('http://', 'https://');
    }
  };
 
  url = url + "?" + "referrer=" + encodeURIComponent(document.referrer) + "&for=" + page; 
  var img = new Image();
  var cleanup = function() {
    img.parentNode.removeChild(img);
    console.log("The following URL was called for statistics purposes during page load:", url);
    httpTohttps();
  }
  img.onload = cleanup;
  img.onerror = cleanup;
